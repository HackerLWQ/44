...
def getOutline(img):
    ...
    imgEdge = cv2.Canny(imgGray,170,230)
    utils.imshow(imgEdge)

    # 提取轮廓
    contours = cv2.findContours(imgEdge,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)[0]
    ...
    return np.array(contours[0] / ratio, dtype=int)
